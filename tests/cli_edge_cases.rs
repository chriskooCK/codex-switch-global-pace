...
fn json_list_adds_global_weekly_without_changing_profile_items() {
    ...
    fs::create_dir_all(home.join(".codex-switch")).unwrap();
    fs::write(home.join(".codex-switch/current"), "pace").unwrap();
    fs::write(
        home.join(".codex-switch/config.toml"),
        "[cache]\nttl = 999999999\n",
    )
    ...
    let stdout = parse_stdout_json(&output);
    assert_eq!(stdout["profiles"][0]["alias"], "pace");
    assert_eq!(stdout["global_weekly"]["included_accounts"], 1);
    assert_eq!(stdout["global_weekly"]["excluded_accounts"], 0);
    assert_eq!(stdout["global_weekly"]["weighting"], "equal");
    assert_eq!(stdout["global_weekly"]["normal_capacity"], 100.0);
