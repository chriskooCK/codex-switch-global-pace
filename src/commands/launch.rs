                ...
                let recovery = if had_original {
                    format!(
                        "The pre-launch auth.json is kept at {}, so nothing is lost: save the \
                         live credentials with `codex-switch-global-pace import {}`, then restore that \
                         backup by hand.",
                        backup.display(),
                        codex_auth.display()
                    )
                    ...
                    format!(
                        "There was no pre-launch auth.json, so deleting this file would lose \
                         these credentials outright: save them with `codex-switch-global-pace import {}`.",
                        codex_auth.display()
                    )
                };
                format!(
...
mod tests {
    ...
    fn restore_keeps_live_credentials_it_could_not_preserve() {
        ...
        assert!(
            msg.contains(&backup.display().to_string())
                && msg.contains("codex-switch-global-pace import"),
    ...
    fn restore_keeps_live_credentials_it_could_not_preserve_without_an_original() {
        ...
        assert_eq!(read_json(&profile_path), staged);
        assert!(format!("{err:#}").contains("codex-switch-global-pace import"));
    }
}
