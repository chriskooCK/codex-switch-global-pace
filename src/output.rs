...
pub struct JsonUsageResult {
    ...
    pub global_weekly: JsonGlobalWeekly,
}

#[derive(Serialize)]
pub struct JsonGlobalWeekly {
    pub pace_percent: Option<f64>,
...
pub fn global_weekly_to_json(summary: &GlobalWeeklySummary) -> JsonGlobalWeekly {
    JsonGlobalWeekly {
        pace_percent: summary.pace_percent,
        reserve_percent_points: summary.reserve_percent_points,
        effective_capacity: summary.effective_capacity,
        normal_capacity: summary.normal_capacity,
        included_accounts: summary.included_accounts,
        excluded_accounts: summary.excluded_accounts,
        weighting: summary.weighting.as_str().to_string(),
        next_reset_at: summary.next_reset_at,
        next_reset_alias: summary.next_reset_alias.clone(),
    }
}
...
mod tests {
    ...
    #[test]
    fn global_weekly_json_is_additive_and_names_weighting() {
        let summary = GlobalWeeklySummary {
            ...
            included_accounts: 3,
            excluded_accounts: 1,
            weighting: crate::usage::GlobalPaceWeighting::Equal,
            next_reset_at: Some(1_800_000_000),
            next_reset_alias: Some("work2".to_string()),
        };
        let value = serde_json::to_value(JsonUsageResult {
            profiles: vec![],
        ...
        assert_eq!(value["global_weekly"]["weighting"], "equal");
        assert_eq!(value["global_weekly"]["included_accounts"], 3);
        assert_eq!(value["global_weekly"]["next_reset_alias"], "work2");
    }
