...
- **Independent Global Pace distribution** — The project is now distributed as
  ...
  `chriskooCK/codex-switch-global-pace`. Release archives, installers,
  self-update metadata, daemon service identifiers, checksums, and build
  provenance all use that independent identity.
- **Global Weekly Pace** — The TUI combines every account that has usable weekly
  quota data into one equally weighted pace. Accounts without usable weekly
  data are excluded, and the displayed reset time is the nearest contributing
  weekly reset.
- **Existing profiles remain reusable** — Both programs intentionally share
  `~/.codex-switch` (or `CODEX_SWITCH_HOME`), so this snapshot can use existing
  profiles without another login. Only one of the two programs' daemons should
  run at a time because they coordinate through that shared state.
- **Direct TUI launch and verified installers** — Running the binary with no
  subcommand opens the TUI, including direct execution of
  `codex-switch-global-pace.exe` on Windows. GitHub release installers verify
  archive checksums, while published release archives also carry GitHub build
  attestations.
...
- **Codex 0.144.1 authentication alignment** — Browser and device login follow the current Codex callback and polling contracts, refresh responses may preserve an omitted ID or refresh token but must explicitly return a new access token, managed authentication policy is enforced, custom CA settings are honored, and `CODEX_HOME` uses the same empty-value fallback as Codex.
